...
function Hazardous(props) {
  const { _id, WasteType, Quantity, DateOfCollection, Location, TransportMethod, Notes } = props.hazardo;

  const history = useNavigate();

  const deleteHandler = async () => {
    await axios.delete(`http://Localhost:5001/hazardous/${_id}`)
      .then(res => res.data)
      .then(() => history("/"))
      .then(() => history("/categorydetailsHza"));
  };
