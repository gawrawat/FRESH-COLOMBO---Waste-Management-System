...
function Recyclable(props) {
  const { _id, WasteType, Quantity, DateOfCollection, Location, TransportMethod, Notes } = props.recyco;

  const history = useNavigate();

  const deleteHandler = async () => {
    await axios.delete(`http://Localhost:5001/recyclable/${_id}`)
      .then(res => res.data)
      .then(() => history("/"))
      .then(() => history("/categorydetailsOr"));
  };

  return (
    <div className="bg-white p-6 rounded-lg shadow-lg mb-6">
      <h1 className="text-2xl font-bold text-gray-800 mb-4">Recyclable Category Details</h1>
